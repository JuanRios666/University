 ...
 */
#define LORAWAN_DEFAULT_DATARATE                    DR_6
...
static int lorawan_init(const struct lorawan_sx12xx_settings* sx12xx_settings, LoRaMacRegion_t region)
{
    ...
    sx12xx_settings.Spi.Nss.pin = sx12xx_settings->spi.nss;
    sx12xx_settings.Reset.pin = sx12xx_settings->reset;
    sx12xx.DIO0.pin = sx12xx_settings->dio0;
    sx12xx_settings.DIO1.pin = sx12xx_settings->dio1;
...
void changeADR(uint8_t adr){
    LmHandlerParams.TxDatarate = adr;
}

uint8_t getADR(){
    return LmHandlerParams.TxDatarate;
}